  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const filteredProducts = allProducts?.filter((product) => product?.category === "Trousers")
    ...
    const favIds = wishlist.map(item => item.id)
    setFavorites(favIds)
  }, [])
  ...
  const toggleFavorite = (product) => {
    ...
    if (isAlreadyFav) {
      ...
      const alreadyExists = wishlist.find(
        item => item.id === uniqueId && item.color === color && item.size === size
      )
    ...
    <>
      ...
      >
        ...
        <section className="relative w-full h-[50vh] overflow-hidden">
          <Image
            src="https://hips.hearstapps.com/hmg-prod/images/mhl-tank-tops-carhartt-2397-lead-68069b90b9875.jpg?crop=0.6666666666666666xw:1xh;center,top&resize=1200:*"
          ...
          <div className="absolute z-50 inset-y-0 right-0 lg:pr-20 flex flex-col justify-center max-w-lg w-full text-right px-4 md:pr-10">
            <h2 className="text-3xl sm:text-5xl md:text-6xl mb-8 leading-tight whitespace-pre-line drop-shadow-xl">
              <span className="text-white">MEN'S TROUSERS</span>
            </h2>
          </div>
        </section>
